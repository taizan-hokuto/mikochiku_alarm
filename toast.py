...
class CloseButton(QPushButton):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(open(mk.resource_path('./css/toast_button.css'), encoding='utf-8').read())
...
class VideoItem(QListWidgetItem):
    ...
    def __init__(self, video):
        ...
        # 読み込み中のプレースホルダ設定
        self.setIcon(QIcon(QPixmap(mk.resource_path("./img/place_holder.bmp"))))
        # バックグラウンドで画像読み込みを開始
        self.thread = QThread()
...
class VideoItemList(QListWidget):
    ...
    def __init__(self, parent, already_open_browser):
        ...
        if already_open_browser:
            self.setStyleSheet(open(mk.resource_path('./css/toast_normal.css'), encoding='utf-8').read())
        else:
            self.setStyleSheet(open(mk.resource_path('./css/toast_hover.css'), encoding='utf-8').read())
...
class Toast(QMainWindow):
    ...
    def initUI(self, parent):
        ...
        btnClose.setIcon(QIcon(QPixmap(mk.resource_path("./img/close_button.png"))))
